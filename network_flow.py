...
def add_end_node(t,graph,node_list):
    ...
    return
def mainOps(node_list_a = None, node_list_b = None):
	graph = Graph()
	...
	concept_model = graph.load_model()
	for elem in node_list_a:
		for each in node_list_b:
			graph.add_edges(elem,each,int(concept_model.similarity(elem.lower(),each.lower())*100))
	for each in node_list_a:
		graph.add_edges('s',each.lower(),100)
	for each in node_list_b:
		graph.add_edges('t',each.lower(),100)
	#print graph
	#print graph.perform_max_flow()
	return graph.peroform_max_flow()
